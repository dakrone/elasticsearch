...
public class TransportThawIndexAction extends TransportMasterNodeAction<ThawIndexRequest, ThawIndexResponse> {
    ...
    protected void masterOperation(final ThawIndexRequest request, final ClusterState state,
        ...
        if (concreteIndices == null || concreteIndices.length == 0) {
            listener.onResponse(new ThawIndexResponse(true));
            return;
        }
        ...
        indexStateService.thawIndex(updateRequest, new ActionListener<ClusterStateUpdateResponse>() {

            @Override
            public void onResponse(ClusterStateUpdateResponse response) {
                listener.onResponse(new ThawIndexResponse(response.isAcknowledged()));
            }
